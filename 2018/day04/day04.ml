...
(* day 04
 *
 * Parse a list of guard records and find guard that slept the most.
 *
 * Usage:
 *     sort < input.txt | ./day04.ml
 *)
...
(* Pairwise seq creating overlapping windows of 2 *)
let pairwise (a : 'a Seq.t) : ('a * 'a) Seq.t =
  let group (pairs : ('a * 'a) Seq.t) (next : 'a) : ('a * 'a) Seq.t =
    match pairs () with
    | Nil -> failwith "empty start sequence"
    | Cons ((_, last), _) -> Seq.cons (last, next) pairs
  in
  match a () with
  | Nil -> Seq.empty
  | Cons (first, b) -> (
      match b () with
      | Nil -> failwith "sequence with one item"
      | Cons (second, rest) ->
          Seq.fold_left group (Seq.return (first, second)) rest )
...
let minute_most_asleep (r : (time * time) list) : int =
...
let main () =
  ...
  |> List.sort (fun goat next -> let (_, goat_asleep) = goat in let (_, next_asleep) = next in Int.compare (time_asleep next_asleep) (time_asleep goat_asleep))
  (* Print ids and times *)
  (* |> List.iter (fun a ->
         let a1, b1 = a in
         Printf.printf "%d\t%d\n" a1 (time_asleep b1)) *)
  |> List.hd
  (* |> function | id, sleep -> Printf.eprintf "id: %d\tminute most asleep: %d\n" id (minute_most_asleep sleep) *)
  (* Print id * common minute asleep... *)
  |> function | id, sleep -> id * (minute_most_asleep sleep)
  |> Printf.printf "%d\n"

let rec windowed (l:'a Seq.t) : ('a * 'a) Seq.t = fun () ->
  match l () with
    | Nil -> Nil
    | Cons (a, l') ->
      match l' () with
      | Nil -> failwith "Uneven number of elements"
      | Cons (a', l'') -> Cons ((a, a'), windowed l'')

(* Take in a list of pairs of minutes (one minute per line) and print the most asleep minute... *)
let sleep_input () =
    stdin |> line_seq_of_channel |> Seq.map int_of_string
    |> windowed
    |> List.of_seq
    |> List.map (function | (a, b) -> ((0, a), (0, b)))
    |> minute_most_asleep
    |> Printf.printf "minute most asleep: %d\n"
